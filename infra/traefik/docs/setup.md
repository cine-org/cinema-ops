...
> Không cài gì. `HelmChartConfig` tên `traefik` ở ns `kube-system` đè values chart của k3s; k3s chạy lại job helm-install để upgrade.
...
Viết `ports.web.redirections` (thiếu cấp `http`) thì chart 40 bỏ qua hoàn toàn: không lỗi, Application vẫn `Synced`, HTTP vẫn 200. Kiểm tra bằng release và pod, không phải grep manifest:
...
DNS only: chỉ trả IP. Bật proxy thì đặt SSL mode **Full (strict)**, vẫn cần cert-manager. Bản ghi A: `cine.io.vn`, `admin.cine.io.vn`, sau này `api.cine.io.vn`.
