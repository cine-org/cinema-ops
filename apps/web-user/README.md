...
Web cho người dùng cuối. Domain `https://cine.io.vn`.

| | |
| --- | --- |
| Image | `ghcr.io/cine-org/setup-monorepo/web-user`, tag đặt ở `envs/staging/kustomization.yaml` |
...
| Chứng chỉ | `cine-io-vn-tls`, cert-manager tự xin từ `letsencrypt-prod` |
...
envs/staging/ingress.yaml          host cine.io.vn + TLS
```
...
**Ingress là đường duy nhất từ Internet.** Service kiểu ClusterIP chỉ tới được từ trong cluster. Annotation `cert-manager.io/cluster-issuer` khiến cert-manager tự tạo `Certificate` và ghi vào Secret `cine-io-vn-tls`; không phải viết `Certificate` bằng tay.
...
```bash
kubectl kustomize apps/web-user/envs/staging
```

## Verify

```bash
kubectl get pods,svc,ingress -n cinema -l app.kubernetes.io/name=web-user
kubectl get certificate cine-io-vn-tls -n cinema
...
```bash
kubectl describe pod -n cinema -l app.kubernetes.io/name=web-user | grep -A10 Events
```
